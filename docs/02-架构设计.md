...
```mermaid
graph TB
  subgraph L1["① 桌面外壳层（1 个 Python 进程 = 知伴.exe）"]
    LAU["launcher.py\n选端口 → 起服务 → 拉 Edge → 退出清理"]
  end
...
| 进程 | 数量 | 角色 | 生命周期 |
|---|---|---|---|
| `知伴.exe`（launcher，Python） | 1 | 主进程。内部以**守护线程**运行 uvicorn（FastAPI），并负责拉起 Edge | 由用户双击启动；Edge 窗口关闭后自行退出 |
...
7. 兜底：若找不到 Edge，改用 `webbrowser.open(url)` 打开默认浏览器，保持服务运行，用户可通过界面「退出知伴」按钮（`POST /api/system/shutdown`）结束。
...
│  ├─ zhiban.spec                    # PyInstaller 配置（datas 打入 web/models，onedir）
...
def data_path(*parts) -> Path:
    ...
    开发态：项目根目录。可用环境变量 ZHIBAN_DATA_DIR 覆盖。"""
```
...
- `zhiban.spec` 必须把 `src/web` 与 `models` 写入 `datas`，把 `src/backend/db/schema.sql` 一并打入。
...
| POST | `/api/system/shutdown` | 优雅关闭（供"退出知伴"按钮；返回后进程退出） |
...
- 支持环境变量覆盖（如 `ZHIBAN_LLM_API_KEY`）用于高级用户/CI，但**不写入任何日志或 DB 明文**。
...
### T14 桌面外壳与打包发布
- **产出**：`src/launcher.py`、`build/zhiban.spec`、`build/build.ps1`、`build/version_info.txt`、`scripts/backup.ps1`、`scripts/restore.ps1`、`README.md`、`LICENSE`、`docs/03-用户手册.md`、`docs/04-管理员手册.md`、`docs/05-模型兼容列表.md`、`docs/06-第三方依赖与许可.md`
- **依赖**：T12、T13
- **判据**：`build.ps1` 退出码 0，产出 `dist/知伴/`（`知伴.exe` + 支持目录）（R-I01）；在**未装 Python/Node/Docker** 的干净 Windows 上解压双击启动、Edge `--app` 窗口打开界面（R-I02）；关闭窗口后进程与端口释放；`data/` 生成于 exe 同级；备份→恢复数据一致（R-I05）。
...
| 2 | **数据目录位置** | 程序目录内 `data/`（绿色便携，整包可拷） | `%APPDATA%\ZhiBan` | 影响"便携"与多副本共存（对应 PRD 待确认#3） |
...
| 10 | **"退出知伴"兜底** | Edge 关闭即退出 + `/api/system/shutdown` 按钮 | 常驻托盘 | 进程收尾体验 |
