# ZhiBan backup script: zhiban.db + files/ + exports/ -> timestamped zip
...
$tmp = Join-Path $env:TEMP ("zhiban-backup-" + $stamp)
New-Item -ItemType Directory -Force -Path $tmp | Out-Null

Copy-Item (Join-Path $dataDir "zhiban.db") $tmp -Force -ErrorAction SilentlyContinue
foreach ($f in @("zhiban.db-wal", "zhiban.db-shm")) {
...
$zip = Join-Path $Dest ("zhiban-backup-" + $stamp + ".zip")
...
Write-Host "Restore with: scripts\restore.ps1 -Zip <path>  (close ZhiBan first)"
