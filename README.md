...
| **朗读（TTS，可选）** | 默认关闭；可选"本地系统语音"（Windows 自带音色，离线零 Key）、"本地神经语音 MeloTTS"（更自然，模型随包，离线零 Key）或"云端 API"（与对话模型**同站点**时端点与 Key 都可留空自动沿用；**异站点**不会误用对话模型的 Key）。设置页有 **测试连接 / 试听**（三种模式都可用）并实时显示"当前生效"引擎；另有 **音色 voice** 一栏（非 OpenAI 系服务商必须填它自己的音色名）；云端失败会**明确报错并给出实际请求 URL 与上游原始错误**，不会静默降级成本地朗读 |
